...
/*...e*/
/*...sgbm_ht_24_3x3     \45\ halftone by 3x3 to r\58\g\58\b bits:0:*/
void gbm_ht_24_3x3(const GBM *gbm, const gbm_u8 *src24, gbm_u8 *dest24, gbm_u8 rm, gbm_u8 gm, gbm_u8 bm)
	{
	int stride = ((gbm->w * 3 + 3) & ~3);
	int x, y;
	...
	for ( y = 0; y < gbm->h - 1; y += 3 )
		{
		const gbm_u8 *src24a  = (src24 += stride);
		const gbm_u8 *src24b  = (src24 += stride);
		const gbm_u8 *src24c  = (src24 += stride);
		      gbm_u8 *dest24a = (dest24 += stride);
		      gbm_u8 *dest24b = (dest24 += stride);
		      gbm_u8 *dest24c = (dest24 += stride);

		for ( x = 0; x < gbm->w - 2; x += 3 )
/*...s3x3 case:24:*/
{
gbm_u16 r,g,b;
gbm_u8 ri,gi,bi;
...
bi = (div9(b) & bm); gi = (div9(g) & gm); ri = (div9(r) & rm);
*dest24a++ = bi; *dest24a++ = gi; *dest24a++ = ri;
b -= bi; g -= gi; r -= ri;
...
bi = (div8(b) & bm); gi = (div8(g) & gm); ri = (div8(r) & rm);
*dest24a++ = bi; *dest24a++ = gi; *dest24a++ = ri;
b -= bi; g -= gi; r -= ri;
...
bi = (div7(b) & bm); gi = (div7(g) & gm); ri = (div7(r) & rm);
*dest24a++ = bi; *dest24a++ = gi; *dest24a++ = ri;
b -= bi; g -= gi; r -= ri;
...
bi = (div6(b) & bm); gi = (div6(g) & gm); ri = (div6(r) & rm);
*dest24b++ = bi; *dest24b++ = gi; *dest24b++ = ri;
b -= bi; g -= gi; r -= ri;
...
bi = (div5(b) & bm); gi = (div5(g) & gm); ri = (div5(r) & rm);
*dest24b++ = bi; *dest24b++ = gi; *dest24b++ = ri;
b -= bi; g -= gi; r -= ri;
...
bi = (div3(b) & bm); gi = (div3(g) & gm); ri = (div3(r) & rm);
*dest24c++ = bi; *dest24c++ = gi; *dest24c++ = ri;
b -= bi; g -= gi; r -= ri;
...
tmp = dest24a; dest24a = dest24b; dest24b = dest24c; dest24c = tmp;
}
/*...e*/
		if ( x < gbm->w - 1 )
/*...s2x3 case:24:*/
