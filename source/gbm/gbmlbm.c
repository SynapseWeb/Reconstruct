...
/*...smapping:0:*/
/*...sget_u32:0:*/
static gbm_u32 get_u32(const gbm_u8 *b)
	{
	return    (gbm_u32) b[3]         +
	        (((gbm_u32) b[2]) <<  8) +
	        (((gbm_u32) b[1]) << 16) +
...
/*...e*/
/*...sget_u16:0:*/
static gbm_u16 get_u16(const gbm_u8 *b)
	{
	return    (gbm_u16) b[1]        +
	        (((gbm_u16) b[0]) << 8) ;
	}
...
priv->body = (gbm_u32) gbm_file_lseek(fd, 0L, SEEK_CUR);
priv->size_body = size;
had_body = GBM_TRUE;
}
/*...e*/
...
/*...e*/
	...
	if ( (offset_end - offset_body) & 1 )
		/* BODY is an odd # of gbm_u8s long! oops, better 'fix' this */
		{
		gbm_u8 b = 0; /* Pad gbm_u8 must be zero */
